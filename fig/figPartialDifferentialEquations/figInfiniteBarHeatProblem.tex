...
\begin{tikzpicture}
\begin{axis}[width=8cm,height=5cm,axis lines*=middle,xlabel={$x$},xtick={-3,-2,-1,1,2,3},xticklabels={$-3$,$-2$,$-1$,$1$,$2$,$3$},ytick={\empty},x label style={at={(current axis.right of origin)},anchor=south west},ylabel={$u(x,t)$},y label style={at={(current axis.above origin)},anchor=south west},ylabel style={rotate=-90},ymax=120]
